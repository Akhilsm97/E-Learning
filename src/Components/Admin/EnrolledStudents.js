...
function EnrolledStudents() {
    ...
    const handleSubmit = async (e, id) => {
        e.preventDefault();
        let studImgData = {};
        if (formData.stud_img) {
            // If stud_img is not empty, include it in requestData
            studImgData = { stud_img: formData.stud_img };
        }else{
            studImgData = { stud_img: '' }; 
        }
        ...
        const response = await axios.put(`http://127.0.0.1:8000/stud_update/${id}/`, requestData,{
            headers: {
    ...
    const[currentPage, setCurrentPage] = useState(1)
        ...
        const handleDelete=((id) =>{
            setShowModal(false)
            fetch(`http://127.0.0.1:8000/stud/${id}/delete/ ` ,
                {method: 'DELETE'})
                .then(()=>{
                    console.log("Deleted")
            ...
            <div class="card mb-3 " style={{maxWidth: 340, height: 225}}>
                <div class="row g-0">
                  ...
                  <div class="col-md-6" style={{marginLeft:-30}}>
                    <div class="card-body">
                      <h5 class="card-title font-weight-bolder"  style={{fontSize:17,}}>{items.Stud_name}</h5>
                      ...
                      <p class="card-text" style={{marginLeft: 50}}><a href="#" data-toggle="modal" data-target="#myModal" onClick={()=>{updateDetails(items.id)}}><img src={Edit} style={{borderRadius: '50%', width: 20}} /></a>&emsp;
                      ...
                      <form id="myForm" onSubmit={(e)=>handleSubmit(e, update.id )}>
                        <div className='row'>
                            <div className='col-md-6 pb-2'>
                                ...
                                <input type='text' className='form-control' name='Stud_name' value={update.Stud_name} onChange={(event)=>handleInputChange(event, 'Stud_name')} />
                            </div>
                            <div className='col-md-6 pb-2'>
                            ...
                            <div className='col-md-6 pb-2'>
                                ...
                                <input type='number' className='form-control' name="percent_mark" value={update.percent_mark} onChange={(event)=>handleInputChange(event, 'percent_mark')}/>
                            </div>
                            <div className='col-md-6 pb-2'>
                            ...
                            <div className='col-md-6 pb-2'>
                                ...
                                <img src={update.stud_img} className='img-thumbnail'  style={{width:100, padding:10}}/>
                                <input type='file' className='form-control' name="stud_img" onChange={handleInputImage} accept='image/*' />
                            </div>
                        </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <p className='font-weight-bolder'> Are you want to delete {update.Stud_name}</p>
